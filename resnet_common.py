...
def block0(x, filters, kernel_size=3, stride=1,
    ...
    if conv_shortcut is True:
        shortcut,_,_ = layers.Conv2D(4 * filters, 1, strides=stride,
                                 name=name + '_0_conv', outputStaticInfo = True)(x)
        shortcut = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                             name=name + '_0_bn')(shortcut)
    else:
        shortcut = x

    x,_,_ = layers.Conv2D(filters, kernel_size, padding='SAME', strides=stride, name=name + '_1_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x)
    x = layers.Activation('relu', name=name + '_1_relu')(x)

    x,_,_ = layers.Conv2D(4*filters, kernel_size, padding='SAME', name=name + '_2_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_2_bn')(x)
...
def block1(x, filters, kernel_size=3, stride=1,
    ...
    x,_,_ = layers.Conv2D(filters, 1, strides=stride, name=name + '_1_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x)
    x = layers.Activation('relu', name=name + '_1_relu')(x)

    x,_,_ = layers.Conv2D(filters, kernel_size, padding='SAME',name=name + '_2_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_2_bn')(x)
    x = layers.Activation('relu', name=name + '_2_relu')(x)

    x,_,_ = layers.Conv2D(4 * filters, 1, name=name + '_3_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_3_bn')(x)
...
def block2(x, filters, kernel_size=3, stride=1,
    ...
    if conv_shortcut is True:
        shortcut,_,_ = layers.Conv2D(4 * filters, 1, strides=stride,
                                 name=name + '_0_conv', outputStaticInfo = True)(preact)
    else:
        shortcut = layers.MaxPooling2D(1, strides=stride)(x) if stride > 1 else x

    x,_,_ = layers.Conv2D(filters, 1, strides=1, use_bias=False,
                      name=name + '_1_conv', outputStaticInfo = True)(preact)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x)
    x = layers.Activation('relu', name=name + '_1_relu')(x)

    x = layers.ZeroPadding2D(padding=((1, 1), (1, 1)), name=name + '_2_pad')(x)
    x,_,_ = layers.Conv2D(filters, kernel_size, strides=stride,
                      use_bias=False, name=name + '_2_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_2_bn')(x)
    x = layers.Activation('relu', name=name + '_2_relu')(x)
...
def block3(x, filters, kernel_size=3, stride=1, groups=32,
    ...
    if conv_shortcut is True:
        shortcut,_,_ = layers.Conv2D((64 // groups) * filters, 1, strides=stride,
                                 use_bias=False, name=name + '_0_conv', outputStaticInfo = True)(x)
        shortcut = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                             name=name + '_0_bn')(shortcut)
    else:
        shortcut = x

    x,_,_ = layers.Conv2D(filters, 1, use_bias=False, name=name + '_1_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x)
    x = layers.Activation('relu', name=name + '_1_relu')(x)
    ...
    x,_,_ = layers.Conv2D((64 // groups) * filters, 1,
                      use_bias=False, name=name + '_3_conv', outputStaticInfo = True)(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_3_bn')(x)
...
def ResNetLoadWeights(model, model_name, weights, include_top) :
    # Load weights.
...
def ResNet(stack_fn,
    ...
    x,_,_ = layers.Conv2D(64, 7, strides=2, use_bias=use_bias, name='conv1_conv', outputStaticInfo = True)(x)
    ...
    if (weights == 'imagenet') and (model_name in WEIGHTS_HASHES):
        ...
        model.load_weights(weights)
    '''
    ResNetLoadWeights(model, model_name, weights, include_top)
